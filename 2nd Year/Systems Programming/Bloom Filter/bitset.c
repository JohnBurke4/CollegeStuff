#include "bitset.h"


// create a new, empty bit vector set with a universe of 'size' items
struct bitset * bitset_new(int size) {
	struct bitset * result;
       	result = malloc(sizeof(struct bitset));
	int sizeInWords = ((size - 1)/64)+1;
	uint64_t * bits = malloc(sizeof(uint64_t*)*sizeInWords);
	for(int i = 0; i < sizeInWords; i++){
		bits[i] = 0;
	}
	result -> bits = bits;
	result -> size_in_words = sizeInWords;
	result -> universe_size = sizeInWords * 64;
	return result;
}

// get the size of the universe of items that could be stored in the set
int bitset_size(struct bitset * this) {
	return this -> universe_size;
}

// get the number of items that are stored in the set
int bitset_cardinality(struct bitset * this){
	int cardinality = 0;
	for (int i = 0; i < this -> size_in_words; i++) {
		uint64_t current = this -> bits[i];
		while (current != 0){
			if (current % 2 == 1){
				cardinality++;
			}
			current /= 2;
		}
	}
	return cardinality;
}

// check to see if an item is in the set
int bitset_lookup(struct bitset * this, int item){
	int pos = item / 64;
	int index = item % 64;
	uint64_t set = this -> bits[pos];
	uint64_t mask = 1UL << 63;
	return ((set << index) & mask ? 1 : 0);
}

// add an item, with number 'item' to the set
// has no effect if the item is already in the set
void bitset_add(struct bitset * this, int item) {
	int pos = item / 64;
	int index = item % 64;
	uint64_t set = this -> bits[pos];
	uint64_t mask = 1ul << (63 - index);
	this -> bits[pos] = (set | mask);
}
  
// remove an item with number 'item' from the set
void bitset_remove(struct bitset * this, int item) {
	int pos = item / 64;
	int index = item % 64;
	uint64_t set = this -> bits[pos];
	uint64_t mask = (~0UL) ^ (1UL << (63 - index)); 

	this -> bits[pos] = (set & mask);

}
  
// place the union of src1 and src2 into dest;
// all of src1, src2, and dest must have the same size universe
void bitset_union(struct bitset * dest, struct bitset * src1,
		  struct bitset * src2) {
	int size = bitset_size(src1);
	for( int i = 0; i < size; i++){
		if (bitset_lookup(src1, i) == 1 || bitset_lookup(src2, i) == 1){
			bitset_add(dest, i);
		} else {
			bitset_remove(dest, i);
		}
	}
}

// place the intersection of src1 and src2 into dest
// all of src1, src2, and dest must have the same size universe
void bitset_intersect(struct bitset * dest, struct bitset * src1,
		      struct bitset * src2) {
	int size = bitset_size(src1);
	for( int i = 0; i < size; i++){
		if (bitset_lookup(src1, i) == 1 && bitset_lookup(src2, i) == 1){
			bitset_add(dest, i);
		} else {
			bitset_remove(dest, i);
		}
	}
}
